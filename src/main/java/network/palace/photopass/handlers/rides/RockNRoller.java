...
public class RockNRoller {
    ...
    FileConfiguration rideData = YamlConfiguration.loadConfiguration(rrcConfigFile);
    ...
    private void requestRRCPhoto(Boolean uploadImgur, List<Player> players, String namesList) {
        MapRender makeMap = new MapRender();
        Core.logInfo(namesList);
        Image img = makeMap.getImageFromAPI("RockNRoller", namesList, config.getString("apiAccess"));
        ...
        if (uploadImgur) {
            try {
                ...
                String info = "Rock 'n' Roller Coaster Starring Aerosmith on " + Core.getInstanceName() + "! Date of Photo: " + DateFormat.formattedTime();
                MongoManager mm = new MongoManager();
                mm.createPhoto(url, playerArr, info);
            }
            catch (Exception e) {
                e.printStackTrace();
                Core.logInfo("[PhotoPass] Error Uploading RRC Picture: " + e.getLocalizedMessage());
            }
        }
    }
}
