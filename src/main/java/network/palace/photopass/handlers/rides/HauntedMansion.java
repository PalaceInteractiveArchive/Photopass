...
public class HauntedMansion {
    ...
    FileConfiguration rideData = YamlConfiguration.loadConfiguration(hmConfigFile);
    ...
    private void requestHmPhoto(Boolean uploadImgur, List<Player> players, String namesList) {
        MapRender makeMap = new MapRender();
        Core.logInfo(namesList);
        Image img = makeMap.getImageFromAPI("HauntedMansion", namesList, config.getString("apiAccess"));
        ...
        if (uploadImgur) {
            try {
                ...
                String info = "Haunted Mansion on " + Core.getInstanceName() + "! Date of Photo: " + DateFormat.formattedTime();
                MongoManager mm = new MongoManager();
                mm.createPhoto(url, playerArr, info);
            }
            catch (Exception e) {
                e.printStackTrace();
                Core.logInfo("[PhotoPass] Error Uploading Haunted Mansion Picture: " + e.getLocalizedMessage());
            }
        }
    }
}
